...
Java_com_example_ndkgifapplication_GifHandler_loadPath(JNIEnv *env, jobject instance,
                                                       jstring gifPath) {
    const char *path = env->GetStringUTFChars(gifPath, 0);
    int err;
    GifFileType *gif = DGifOpenFileName(path, &err);

    err = DGifSlurp(gif);

    env->ReleaseStringUTFChars(gifPath, path);
    return gif;
}extern "C"
JNIEXPORT jint JNICALL
Java_com_example_ndkgifapplication_GifHandler_getWidth__J(JNIEnv *env, jobject instance,
                                                          jlong ndkGif) {
    return ((GifFileType *) ndkGif)->SWidth;
}extern "C"
JNIEXPORT jint JNICALL
Java_com_example_ndkgifapplication_GifHandler_getHeight__J(JNIEnv *env, jobject instance,
                                                           jlong ndkGif) {
...
JNIEXPORT jint JNICALL
Java_com_example_ndkgifapplication_GifHandler_getLength__J(JNIEnv *env, jobject instance,
                                                           jlong ndkGif) {
...
Java_com_example_ndkgifapplication_GifHandler_recycleGif(JNIEnv *env, jobject instance,
                                                         jlong ndkGif) {
